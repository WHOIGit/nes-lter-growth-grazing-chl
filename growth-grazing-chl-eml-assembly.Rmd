...
```{r}
# data provided by pierre
gg_pierre <- read_excel(path = paste0(dir, "/", pierre_data, ".xlsx"), 
                  sheet = "DataTable", na = "NA") %>% select(mv_cols)
# cleaned
gg_edi <- growgraze_EDI %>% select(mv_cols) %>%
    rename_at(mv_cols, ~ paste0(mv_cols, "_clean"))

# merge
# create logical check, ignoring missing values
